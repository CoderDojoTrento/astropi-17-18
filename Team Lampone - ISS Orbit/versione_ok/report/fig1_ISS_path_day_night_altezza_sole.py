...
for file_immagine in lista_imgs[:]:
    ...
    print("%d,%s,%s,%s,%s,%s" % (progressivo,file_immagine,degrees(ISS.sublat), degrees(ISS.sublong), ISS.elevation, angolo_sole))
    progressivo += 1
    
    ## diametro vista oblo':400 Km 
    (x,y)=map(degrees(ISS.sublong),degrees(ISS.sublat))
    circle = Circle(xy=(x,y),radius=400000/2, fill=False, color='black' if (angolo_sole<0) else cmap(angolo_sole/70.0))
